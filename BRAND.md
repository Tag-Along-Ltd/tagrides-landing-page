...
For any context that doesn't accept SVG — GitHub repo previews, slide decks, email signatures, print, Twitter/LinkedIn shares.

| File | Dimensions | Notes |
| --- | --- | --- |
| `mark.png` | 1024 × 1024 | Mark centered on white. Use on any light surface |
| `lockup-horizontal-large.png` | 1600 × 400 | Mark + wordmark, white background. Slide decks, email signatures |
| `og-image.png` | 1200 × 630 | The PNG used as Open Graph image — what shows when the landing-page URL is shared |
| **`github-social.png`** | **1280 × 640** | **Upload as the GitHub repo Social Preview (Settings → Options → Social preview)** |

PNG renders are generated by rendering temporary HTML files inside `/public/` and screenshotting at the target dimensions via Playwright. To regenerate after a brand change, see `scripts/render-brand-rasters.md` (TBD).
