...
const MODES = [
  {
    ...
    icon: Users,
    accent: 'primary',
    body: 'The wedge. A rider joins a driver already heading that way, shares the route, and pays only for the leg they use — closer to danfo logic than private-car pricing.',
    best: [
      'Daily commutes',
      'Regular trips',
      'Anyone who’d rather pay less than have the car to themselves',
    ],
  },
  {
    key: 'direct',
    ...
    icon: User,
    accent: 'accent',
    body: 'A dedicated whole-car option is planned for a later release. It is not available in the live journey today.',
    best: [
      'Hospital trips',
      'Late nights',
      'Sensitive meetings',
      'Anything you want to do privately',
    ],
  },
];
      ...
      <div className="mx-auto max-w-6xl px-6 py-14 md:py-28">
        <Reveal>
          ...
          <h2 className="mt-3 max-w-3xl font-display text-3xl font-bold leading-tight tracking-tight text-foreground md:text-5xl">
            Share the route. Share the fare.
          </h2>
        </Reveal>
        ...
        <RevealStagger className="mt-14 grid gap-6 md:grid-cols-2">
          {MODES.map((mode) => {
            ...
            return (
              ...
              >
                <div
                  className={`inline-flex size-12 items-center justify-center rounded-xl ring-1 ${iconWrap}`}
                >
                ...
                <h3
                  className={`mt-2 font-display text-3xl font-bold tracking-tight ${titleAccent}`}
                >
                  {mode.title}
                </h3>
